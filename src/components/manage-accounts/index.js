...
export default function ManageAccounts() {
  ...
  async function getAllAccounts() {
    const res = await fetch(
      `http://localhost:8000/user/${session?.user?.uid}`,
      {
        method: 'GET',
        headers: {
  ...
  async function handleRemoveAccount(getItem) {
    const res = await fetch(`http://localhost:8000/user/${getItem._id}`, {
      method: 'DELETE',
    });
  ...
  async function handlePinSubmit(value, index) {
    console.log(showPinContainer.account.name, 'aa');
    console.log(value, 'value');
    console.log(session?.user?.uid, 'uid');
    setPageLoader(true);
    const response = await fetch('http://localhost:8000/user/login', {
      ...
      body: JSON.stringify({
        uid: session?.user?.uid.toString(),
        name: showPinContainer.account.name,
        pin: value,
      }),
    });
